...
want = pdsql('''
   select
     ...
     ,lot
   from
      (select *, row_number() over (partition by var_1) as RECNO from have )
   where
      RECNO % 3 in (1,2)
''');
print(want);
...
my_data %>% slice_head(n = 2,by = c(VAR_1, VAR_2, VAR_3))
...
Obs    VAR_1    VAR_2    VAR_3    LOT    PARTITION
